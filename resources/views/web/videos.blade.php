...
<div class="posts">
    ...
    @foreach ($videos as $video)
        <div class="post_resume">
            ...
            <div class="image">
                <a href="{{ route('web.video', [$type,$video->id]) }}">
                    <img class="lazy" data-src="{{$video->thumbnail($video->video->url) }}" alt="{{ $video->translate('es')->title }}" >
                </a>
            </div>
            <div class="text">
            <a href="{{ route('web.video', [$type,$video->id]) }}"><h2>{{ $video->translateOrDefault(\App::getLocale())->title}}</h2></a>
            <div class="content">{!!  strip_tags(\Illuminate\Support\Str::words($video->translateOrDefault(\App::getLocale())->content, 25,'...')) !!}</div>
            </div>
        </div>
    @endforeach
    @else
