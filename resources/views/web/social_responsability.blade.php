...
@section('title')
@lang('menu.social_responsability')
@endsection
@section('css')
<style type="text/css">
    @font-face {
...
<section>
    <div class="content">
    <h2>{{ $text->translateOrDefault(App::getLocale())->title }}</h2>
    {!! $text->translateOrDefault(App::getLocale())->subtitle !== '' ? '<h3>'.$text->translateOrDefault(App::getLocale())->subtitle.'</h3>' : '' !!}
    <div class="text">
        {!! $text->translateOrDefault(App::getLocale())->content !!}
    </div>
    </div>
    <div class="image">
        <picture>
            <source class="lazy" data-srcset="{{ asset('storage/images/'.$text->image->webp) }}" type="image/webp">
            <source class="lazy" data-srcset="{{ asset('storage/images/'.$text->image->url) }}" type="image/{{ $text->image->extension }}">
            <img class="lazy" data-src="{{ asset('storage/images/'.$text->image->url) }}" alt="{{ Str::slug($text->translateOrDefault(App::getLocale())->title) }}">
        </picture>
    </div>
</section>
@endif
