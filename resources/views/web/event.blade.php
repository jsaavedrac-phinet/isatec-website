...
<section>
    <div class="post">
        <h2>{{ $event->translateOrDefault(\App::getLocale())->title}}</h2>
        @if ($event->translateOrDefault(\App::getLocale())->subtitle != '')
        <h3>{{ $event->translateOrDefault(\App::getLocale())->subtitle}}</h3>
        @endif
        <div class="image">
            <picture>
                <source class="lazy" data-srcset="{{ asset('storage/images/'.$event->image->webp) }}"
                    type="image/webp">
                <source class="lazy" data-srcset="{{ asset('storage/images/'.$event->image->url) }}"
                    type="image/{{$event->image->extension}}">
                <img class="lazy" data-src="{{asset('storage/images/'.$event->image->url) }}"
                    alt="{{ $event->translate('es')->title }}">
            </picture>
        </div>
        <div class="text">
            {!! $event->translateOrDefault(\App::getLocale())->content !!}
        </div>
    </div>
