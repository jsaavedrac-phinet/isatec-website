...
<section>
    ...
    <div class="image">
        <picture>
            <source class="lazy" data-srcset="{{ asset('storage/images/'.$service->image->webp) }}" type="image/webp">
            <source class="lazy" data-srcset="{{ asset('storage/images/'.$service->image->url) }}" type="image/{{ $service->image->extension }}">
            <img class="lazy" data-src="{{ asset('storage/images/'.$service->image->url) }}" alt="{{ Str::slug($service->title) }}">
        </picture>
    </div>
</section>
{{-- @if (count($contents) > 0)
        ...
        <section>
            ...
            <div class="image">
                <picture>
                    ...
                    <img class="lazy" data-src="{{ asset('storage/images/'.$item->image->url) }}" alt="{{ Str::slug($item->title) }}">
                </picture>
            </div>
        </section>
    @endforeach
