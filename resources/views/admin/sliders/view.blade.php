...
<div class="header-content">
	<div class="icon-content"><span class="icon-profile"></span></div>
	<div class="title-content">Sliders</div>
	<div class="actions-content">
		<a class="btn btn-add" href="{{ route('admin.slider.create') }}">
			<div class="icon-action"><i class="fas fa-plus-circle"></i></div>
			<div class="title-action">Nuevo</div>
		</a>
		<a class="btn btn-sort" href="{{ route('admin.slider.sort') }}">
			<div class="icon-action"><i class="fas fa-list-ol"></i></div>
			<div class="title-action">Ordenar</div>
		</a>
	</div>
</div>
<div class="body-content">
	@if (count($sliders) >0)
	<table>
		<caption>Hay un total de {{ count($sliders) }} slider(s) registrado(s)</caption>
		<thead>
			<tr>
		...
		<tbody>
			...
			<tr>
				<td class="imagen">
					<picture>
						<source src-set="{{asset('storage/images/'.$slider->image->webp)}}" type="image/webp">
						<source src-set="{{asset('storage/images/'. $slider->image->url)}}"  type="image/{{$slider->image->extension}}">
						<img src="{{asset('storage/images/'. $slider->image->url)}}">
					</picture>
                </td>
                <td class="imagen">
                    <picture>
                        <img src="{{asset('storage/styles/slider/'.$slider->style)}}" alt="">
                    </picture>
                </td>
				<td class="acciones">
					<a class="btn btn-edit" href="{{ route('admin.slider.edit',[$slider->id]) }}" title=""><i class="fas fa-edit"></i>&nbsp;<label>Editar</label>
                    </a>
					<form id="delete-{{$slider->id}}">
					<input type="hidden" name="id" value="{{$slider->id}}">
					<input type="hidden" name="operacion" value="delete">
					<button class="btn btn-delete" type="submit" numero="delete-{{$slider->id}}"><i class="fas fa-trash-alt"></i>&nbsp;<label>Eliminar</label></button>
					</form>
				</td>
			</tr>
			@endforeach
