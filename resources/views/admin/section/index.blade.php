...
<div class="header-content">
	...
	<div class="actions-content">
		<a class="btn btn-add" href="{{ route('admin.section.create', [$name,$image,$content]) }}">
			<div class="icon-action"><i class="fas fa-plus-circle"></i></div>
			<div class="title-action">Nuevo</div>
		</a>
		<a class="btn btn-sort" href=" {{ route('admin.section.sort', [$name,$image,$content]) }}">
...
<div class="body-content">
	@if (count($section) >0)
	<table>
		<caption>Total : {{ count($section) }}</caption>
		<thead>
			<tr>
                @if ($show_image)
                <th class="imagen">IMAGEN</th>
                @endif
                <th class="imagen">TITULO</th>
		...
		<tbody>
			...
			<tr>
				...
				<td class="acciones">
					<a class="btn btn-edit" href="{{ route('admin.section.edit', [$name,$image,$content,$item->id]) }}"><i class="fas fa-edit"></i>&nbsp;<label>Editar</label></a>
                     @if ($show_content)
                    {{-- <a class="btn btn-sections" href="{{ route('admin.section.index', $path_sections."-".$item->id) }}" >Contenido</a> --}}
                    <a class="btn btn-add" href="{{ route('admin.banner.create', $item->slug) }}" ><i class="fas fa-image"></i>&nbsp;Banner</a>
                    @endif
