...
@section('title',$title)
@section('tab-parent')
<a class="active" href="{{ route('admin.post.index',$name) }}" title="">Listado de {{ $name }}</a>
@endsection
@section('content')
...
<div class="body-content">
	<form action="" class="form" id="1">
        ...
        <textarea name="content" class="trumbowyg">{{ $post->content }}</textarea>
        </div>
        <div class="data-option">
        ...
        <div class="data-option">
            ...
            <div class="col20">
                @php
                    ...
                    foreach ($tags as $tag) {
                        ...
                        array_push($ntags, $nuevo);
                    }
                    foreach ($post->tags as $comparar) {
                        for ($i=0; $i < sizeof($ntags) ; $i++) {
                            if ($comparar->id === $ntags[$i]['id']) {
                                $ntags[$i]['selected']= "selected";
                            }
                        }
            ...
            <select class="tags" multiple="multiple" name="tags[]">
                    @foreach ($ntags as $ntag)
                        @if ($ntag['selected'] === "selected")
                            <option value="{{$ntag['id']}}" selected="selected">
						@else
							<option value="{{$ntag['id']}}" >
						@endif
							{{$ntag['name']}}</option>
					@endforeach
                </select>
        ...
        <input type="text" name="date" class="datetime" value="{{ $post->date }}">
        </div>
		<div class="data-option">
...
<style type="text/css">
    ...
    .col20{width: 100%;}
    .select2-container--default .select2-selection--multiple{font-size: 1.5em;}
    .select2-container--default .select2-selection--multiple .select2-selection__choice__remove{font-size: 1em; margin:0; width: auto;}
    .select2-container--default .select2-selection--multiple .select2-selection__rendered li{font-size: 1.7em;}
