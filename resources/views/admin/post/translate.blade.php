...
@section('title',ucfirst($type) . ": Traduccion")
@section('css')
<style type="text/css">
...
@endsection
@section('tab-parent')
<a class="active" href="{{ $parent_page }}" title="">{{ ucfirst($type) }}</a>
@endsection
@section('content')
<div class="header-content">
<div class="title-content" style="text-overflow:ellipsis; overflow:hidden; white-space:pre;">{{ ucfirst($type) }} : Traduccion {!! $post->translate('es')->title ? $post->translate('es')->title : $post->translate('es')->content !!}</div>
</div>
<div class="body-content">
	<form action="" class="form" id="1">
        <input type="hidden" name="type" value="{{ $type }}">
        <input type="hidden" name="id" value="{{ $post->id }}">
        <input type="hidden" name="operation" value="translate">
        <div class="data-option">
			<label for="title">T&iacute;tulo</label>
			<input type="text" name="title" id="title" value="{{ $post->translate('en') ? $post->translate('en')->title : '' }}">
		</div>
		<div class="data-option">
			<label for="content">Contenido :</label>
			<textarea name="content" class="trumbowyg">{{ $post->translate('en') ? $post->translate('en')->content : '' }}</textarea>
        </div>
		<div class="data-option">
